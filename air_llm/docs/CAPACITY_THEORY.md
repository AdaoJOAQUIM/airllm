...
## 5. Attack plans for the two target theorems

### Theorem K (target): universal storage capacity
*For gradient-trained parametric families at convergence with sufficient
exposures, extractable information per parameter tends to a constant
κ ≈ 2 bits, independent of architecture; robust to 8-bit weight
encoding, destroyed below ~4 bits.*

**The buried classical anchor.** The constant is not new. Cover's
function-counting theorem (1965) gives a perceptron with n weights a
capacity of exactly **2n patterns**; Gardner's replica computation (1988)
gives storage capacity α_c = 2 per synapse for random labels; and
Talagrand later made parts of this spin-glass program *rigorous*
(Abel Prize 2024). One random-label pattern ≈ one bit at capacity, so
classical theory already proves **κ = 2 bits/param for the simplest
architecture**. Allen-Zhu & Li's transformers (~2), and this repo's MLP
(≥ 2.10, docs/EXPERIMENTS.md E2), look like *echoes of Cover–Gardner
universality* at depth. Nobody has connected these literatures.

**Why it is not yet proved.** (1) Gardner's calculation is architecture-
specific; universality across depth needs an invariance principle, and
rigorous replica methods (Talagrand, Panchenko) currently reach shallow
or spherical models only. (2) SGD's stationary distribution on singular
landscapes has no workable description — classical learning theory (VC,
Rademacher) is orders of magnitude off, and the landscape is genuinely
singular (degenerate Hessians), which is Watanabe's singular learning
theory territory (RLCT via Hironaka resolution — computed only for tiny
nets). (3) Our E2 killed the single-step channel model: the right object
is the mutual information of the whole *trajectory*, a channel with
memory and feedback — where Shannon theory is thin. The nearest rigorous
brick: Xu & Raginsky (2017) bound I(W; D) for SGLD, where noise is
explicit.

**The laureate's route (three provable stages).**
1. *Solvable regime first.* Replace SGD by SGLD/Gibbs; on the
   fact-memorization task the posterior is an explicit spin-glass-like
   measure. Compute κ(β) exactly by Gardner-style methods made rigorous
   à la Talagrand. Deliverable: κ for one deep family, with proof.
2. *Universality.* Interpolation/Lindeberg swapping between
   architectures (the random-matrix universality playbook) to show κ's
   architecture-independence at fixed parameter count and exposure.
3. *Transfer to SGD.* Effective-temperature equivalences between SGD
   noise and Langevin noise; quantization robustness then falls out as
   rate–distortion of the Gibbs measure (predicting the 8-vs-4-bit
   cliff of E1).

### Theorem P (target): programmatic compression of algorithmic capability
*For capability families with algorithmic structure, a
reasoner+interpreter+growing-library system achieves LCDL polylogarithmic
in the fact-equivalent size, while pure-weight systems pay Theorem K's
linear rate.*

**Why it is not yet proved.** (1) "Algorithmic structure" must be
formalized without circularity — the natural currency is Levin's Kt
...
**The laureate's route.** Prove the *restricted* theorem first: for task
families generated by a bounded grammar (k rules), Levin search
augmented with library compression achieves cumulative search time and
code length polylog in family size — a cumulative-speedup theorem in the
lineage of Schmidhuber's OOPS asymptotic-optimality results, but
finitary and quantitative. Then push the frontier toward general Kt
